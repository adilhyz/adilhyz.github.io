...
vim /etc/locale.gen 
```
![locale.gen](/posts/arch-linux-full-install-with-bspwm/localegen.gif "After that, create a locale:")
...
![pacman.conf](/posts/arch-linux-full-install-with-bspwm/pacmanconf.gif "Then change the config ```/etc/pacman.conf``` to add [Color](#pacman.conf) and [ILoveCandy](#pacman.conf)")
